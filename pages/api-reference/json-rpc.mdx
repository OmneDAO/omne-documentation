...
| SDK role | Deployed network | Chain ID |
|---|---|---|
| `primum` | Local dev (`ws://localhost:8545`; node binary defaults to `:9944`) | 0 |
| `testum` | **Testum** testnet (not yet live) | 1 |
| `principalis` | **Primum** mainnet (not yet live) | 42 |

To connect to the live **Ignis devnet**, use the URL directly: `wss://rpc.ignis.omnechain.network`. Ignis is a deployed public network; it is not reachable via `createClient(role)` in `@omne/sdk@0.3.x`.
