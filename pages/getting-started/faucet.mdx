...
The Ignis devnet faucet mints free testnet OMC and OGT to any valid `om1z...` address. No API keys. No sign-up. A 60-second cooldown per address to prevent abuse.
...
curl -s -X POST https://rpc.ignis.omnechain.network \
  ...
  -d '{"jsonrpc":"2.0","method":"faucet_request","params":["om1z_your_address_here"],"id":1}' | jq .
```
...
curl -s -X POST https://rpc.ignis.omnechain.network \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","method":"omne_getBalance","params":["om1z_your_address_here"],"id":2}' | jq .
```
...
| `invalid_address` | Address is not a valid `om1z…` (bech32m) address | Verify the address with `isValidOmneAddress()` from the SDK |
