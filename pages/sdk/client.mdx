...
const local   = createClient('primum')       // local dev role — ws://localhost:8545
const testnet = createClient('testum')       // Testum testnet role (not yet live)
const mainnet = createClient('principalis')  // Primum mainnet role (not yet live)
```

import { Callout } from 'nextra/components'

<Callout>
  **SDK role keys vs. deployed-network codenames.** The factory's keys are **environment roles** in SDK code. The **deployed networks** use their own codenames: **Ignis** (devnet, live), **Testum** (testnet, forming), **Primum** (mainnet, upcoming). To connect to the live Ignis devnet, pass the URL to the constructor directly: `new OmneClient('wss://rpc.ignis.omnechain.network')`.
</Callout>
...
```typescript
const balance = await client.getBalance('om1z...')

balance.address      // "om1z..."
...
```typescript
const nonce = await client.getTransactionCount('om1z...')
```
...
// https://omnescan.com/tx/${receipt.transactionId}
```
...
```typescript
const balance = await client.getTokenBalance('contract_...', 'om1z...')
```
...
```typescript
const perf = await client.getValidatorPerformance('om1zvalidator...')
```

### `calculateValidatorRewards(address, baseReward?)`

```typescript
const rewards = await client.calculateValidatorRewards('om1zvalidator...')
